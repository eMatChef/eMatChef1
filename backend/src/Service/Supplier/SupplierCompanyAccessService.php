...
class SupplierCompanyAccessService
{
    ...
    public function requireTemplatesAccess(User $user, string $companyId): SupplierCompany
    {
        $this->assertSupplierCompanyAccess($user, $companyId);
        ...
        $company = $membership->getSupplierCompany();
        if (!$this->companyHasCapability($company, SupplierCompany::CAPABILITY_TEMPLATES)) {
            throw new AccessDeniedHttpException('Templates-Capability ist für diese Firma nicht aktiviert');
        }
